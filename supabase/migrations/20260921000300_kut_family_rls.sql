...
alter table public.llf_assets   enable row level security;
...
drop policy if exists "llf_assets public read" on public.llf_assets;
create policy "llf_assets public read"
  on public.llf_assets for select
  to anon, authenticated
  using (true);
