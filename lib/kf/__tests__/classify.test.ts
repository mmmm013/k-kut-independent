...
test('a row with no declared type is classified from its hints', () => {
  assert.equal(classifyUnitType({ title: 'LineFeel · Ch1' }), 'sK');
...
test('each unit type routes to its own player', () => {
  ...
  assert.equal(unitHref('sK', 'x1'), '/llf/x1');
...
test('LineFeel is a one-liner, which is an sK — not a peer container', () => {
  assert.equal(classifyUnitType({ delivery_unit_type: 'LLF' }), 'sK');
  assert.equal(classifyUnitType({ unit_type: '1LNR' }), 'sK');
  assert.equal(classifyUnitType({ delivery_unit_type: 'sK' }), 'sK');
  assert.equal(classifyUnitType({ title: 'SHORT-KUT hook' }), 'sK');
});
